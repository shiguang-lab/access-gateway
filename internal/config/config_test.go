...
func TestNASRoutesValidateAndPreserveProductionContracts(t *testing.T) {
	routes, err := filepath.Abs(filepath.Join("..", "..", "deploy", "nas", "routes.json"))
	if err != nil {
		t.Fatal(err)
	}
	t.Setenv("AUTH_SERVICE_URL", "http://auth-service:8081")
	...
	cfg, err := Load()
	if err != nil {
		t.Fatalf("NAS routes are invalid: %v", err)
	}
	requiredHosts := map[string]bool{
		"shiguanglab.com": false, "www.shiguanglab.com": false, "opc.shiguanglab.com": false,
		"console.shiguanglab.com": false, "model-gateway.shiguanglab.com": false,
		"ai.shiguanglab.com": false, "doc.shiguanglab.com": false,
		"analytics.shiguanglab.com": false, "static.shiguanglab.com": false,
		"sso.shiguanglab.com": false,
	}
	healthPaths := map[string]bool{"/api/health": false, "/api/healthz": false, "/api/livez": false, "/api/readyz": false}
	websiteDevicePages := 0
	modelIdentityPriority := 0
	staticReadOnly := false
	for _, route := range cfg.Routes {
		if _, ok := requiredHosts[route.Host]; ok {
			requiredHosts[route.Host] = true
		}
		if (route.Host == "shiguanglab.com" || route.Host == "www.shiguanglab.com") && route.PathPrefix == "/oauth/device" && route.ExactPath && route.Public && route.Upstream == "http://website:8080" {
			websiteDevicePages++
		}
		if route.Host == "model-gateway.shiguanglab.com" && route.ForwardIdentityAssertion {
			modelIdentityPriority = route.Priority
		}
		if route.Host == "ai.shiguanglab.com" {
			if _, ok := healthPaths[route.PathPrefix]; ok && route.ExactPath && route.Public {
				healthPaths[route.PathPrefix] = true
			}
		}
		if route.Host == "static.shiguanglab.com" && route.PathPrefix == "/" && route.AddPathPrefix == "/public-media" && strings.Join(route.AllowedMethods, ",") == "GET,HEAD" {
			staticReadOnly = true
		}
	}
	for host, found := range requiredHosts {
		if !found {
			t.Errorf("NAS routes missing host %q", host)
		}
	}
	if websiteDevicePages != 2 {
		t.Errorf("OAuth device page routes = %d, want 2", websiteDevicePages)
	}
	if modelIdentityPriority <= 0 {
		t.Error("model gateway identity assertion route must override ordinary API routes")
	}
	for path, found := range healthPaths {
		if !found {
			t.Errorf("missing exact public OmniRoute health path %q", path)
		}
	}
	if !staticReadOnly {
		t.Error("static media route must remain read-only and rewrite into /public-media")
	}
}
